...
class OrderAPIController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($payment['payment']) && $payment['payment']['method']) {
              ...
              if ($payment['payment']['method'] == 'Cash on Delivery') {
                ...
                return $this->bankakPayment($request);
     ...
     * @return \Illuminate\Http\JsonResponse|mixed
     */
    private function bankakPayment(Request $request){
        ...
        try {
            $order = $this->orderRepository->create(
                $request->only('user_id', 'order_status_id','tax', 'delivery_address_id', 'trx_id', 'delivery_fee', 'hint')
            );
            Log::info($order);
            ...
            foreach ($input['products'] as $productOrder) {
                ...
                $amount += $productOrder['price'] * $productOrder['quantity'];
                     ...
                     /////////////// add identification and RandomId ///
                           $RandomId=rand(0, 9999999);
                           $identification_market= $this->productOrderRepository->create($productOrder);
                           $identification=$identification_market->product->market->name;
                           switch ($identification) {
                               case "Bahri Groceries":
                                   $identification = "BG".$RandomId;
                                   break;
                               case "Omdurman Groceries":
                                   $identification = "OG".$RandomId;
                                   break;
                               case "Wholesale Groceries Bahri":
                                   $identification = "WGB".$RandomId;
                                   break;
                              case "Wholesale Groceries Omdurman":
                                   $identification = "WGO".$RandomId;
                                   break;       
                               default:
                                    $identification = "".$RandomId;
            ...
            $this->orderRepository->update(['random_number' =>$identification], $order->id);
