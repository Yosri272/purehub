    ...
    <div class="card-body login">
        ...
        <form action="{{ url('/login') }}" method="post">
          ...
          <div class="input-group mb-3">
                      <input value="{{ old('email') }}" type="email" class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }} small-input" name="email" placeholder="{{ __('auth.email') }}" aria-label="{{ __('auth.email') }}">
